...
class Bnb {
    ...
    async startSession(authorization, xsrfToken, bnbfreeSession, payload) {
    ...
    async claimBalance(authorization, xsrfToken, bnbfreeSession) {
    ...
    async getUserData(no, authorization, xsrfToken, bnbfreeSession) {
        const url = "https://bnbfree.app/user";
        const headers = this.headers(authorization, xsrfToken, bnbfreeSession);
    
        try {
            const response = await axios.get(url, { headers });
            console.log(colors.cyan(`========== Tài khoản ${no + 1} ==========`));
            const { balance, miner } = response.data;
            ...
            if (currentTime > claimedAt) {
                const claimResult = await this.claimBalance(authorization, xsrfToken, bnbfreeSession);
                if (claimResult) {
    ...
    async main() {
        ...
        for (let i = 0; i < this.authorizations.length; i++) {
            ...
            await this.startSession(authorization, xsrfToken, bnbfreeSession, payload);
            const nextClaimTime = await this.getUserData(i, authorization, xsrfToken, bnbfreeSession);
